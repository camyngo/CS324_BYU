...
void eval(char *cmdline)
{
    ...
    char *argv[MAXARGS];
    int command[MAXARGS];
    int stdin_dir[MAXARGS];
    int stdout_dir[MAXARGS];
    int pid[MAXARGS];

    FILE *file_name[MAXARGS];
    int bg = parseline(cmdline, argv); //parseline returns true if its a background
    if (!builtin_cmd(argv))
    {
        int num_cmd = parseargs(argv, command, stdin_dir, stdout_dir);
        int pipe_num[num_cmd - 1][2]; //PIPES

        for (int i = 0; i < num_cmd; ++i)
        {
            //set up pipes before fork and excluding last
            if (i != num_cmd - 1)
            {
                pipe(pipe_num[i]); //PIPES
            }
            pid[i] = fork();

            //setpgid(pid, pgid) where pgid is the pid of the first child processs in the pipelinepid is the process pid
            if (pid[i] == 0)
            {
                //child file
                setpgid(pid[i], pid[0]);
                ...
                if (i > 0)
                { //not first
                    dup2(pipe_num[i - 1][0], STDIN_FILENO);

                    close(pipe_num[i - 1][0]);
                }
                if (i != num_cmd - 1)
                {
                    dup2(pipe_num[i][1], STDOUT_FILENO);
                ...
                if (stdin_dir[i] != -1)
                {
                    file_name[2 * i] = fopen(argv[stdin_dir[i]], "r");
                    int fileIn = fileno(file_name[2 * i]);
                    dup2(fileIn, 0);
                }
                if (stdout_dir[i] != -1)
                {
                    int fileIndex = ((2 * i) + 1);
                    file_name[fileIndex] = fopen(argv[stdout_dir[i]], "w");
                    int fileOut = fileno(file_name[fileIndex]);
                    dup2(fileOut, 1);
                }

                execv(argv[command[i]], &argv[command[i]]);
        ...
        if (!bg)
        {
            ...
            {
                // check to make sure to errors pid
...
int builtin_cmd(char **argv)
{
    char *exitTerminal = "quit";
    if (!strcmp(argv[0], exitTerminal))
    {
