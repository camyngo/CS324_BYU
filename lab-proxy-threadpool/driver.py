    ...
    def run(self):
        ...
        for num, (classes, pts_for_group) in enumerate(self.test_classes):
            ...
            for cls in classes:
                ...
                print('      Result: %d/%d%s' %
                      (p.successes, p.attempts, mem_msg))
        ...
        if self.clean_shutdown is not None:
            ...
            clean_shutdown = self.clean_shutdown * \
                (clean_shutdown/possible_clean_shutdown)
            pts += clean_shutdown
...
class ProxyTest:
    ...
    def __init__(self, proxy_host='localhost', proxy_port=None,
        ...
        self.logger.info(
            'Created temporary directory for files downloaded from the proxy: %s.', self.proxy_dir)
        self.noproxy_dir = tempfile.mkdtemp(prefix='noproxy_', dir='.')
        self.logger.info(
            'Created temporary directory for files downloaded directly from the server: %s.', self.noproxy_dir)
        fd, self.valgrind_log_file = tempfile.mkstemp(prefix='log_', dir='.')
        self.logger.info(
            'Created temporary file for valgrind output: %s.', self.valgrind_log_file)
        os.close(fd)
    ...
    def get_num_processes(cls, pid):
        cmd = ['ps', '--no-headers', '-o', 'pid',
               '--pid', str(pid), '--ppid', str(pid)]
        try:
    ...
    def check_mode(self, mode):
        ...
        if self.num_processes_realtime > self.num_processes_pre:
            if mode == 'multiprocess':
                ...
                level = logging.ERROR
                status = False
            self.logger.log(level, 'Processes are being forked on the fly: (%d > %d)' % (
                self.num_processes_realtime, self.num_processes_pre))
        if self.num_processes_pre > 1:
            if mode == 'processpool':
                ...
                level = logging.ERROR
                status = False
            self.logger.log(level, 'A pool of %d processes was created a program start.' % (
                self.num_threads_pre - 1))
        if self.num_threads_realtime > self.num_threads_pre:
            if mode == 'multithread':
                ...
                level = logging.ERROR
                status = False
            self.logger.log(level, 'Threads are being spawned on the fly: (%d > %d)' % (
                self.num_threads_realtime, self.num_threads_pre))
        if self.num_threads_pre > 2:
            if mode == 'threadpool':
                ...
                level = logging.ERROR
                status = False
            self.logger.log(level, 'A pool of %d threads was created a program start.' % (
                self.num_threads_pre - 2))
        return status
    ...
    def kill_tiny(self):
        ...
        self._cleanup_processes(
            kill_tiny=True, kill_proxy=False, kill_nop=False)

    def cleanup_files(self):
        self.logger.info('Removing temporary files: %s, %s, and %s' % (
            self.proxy_dir, self.noproxy_dir, self.valgrind_log_file))
    ...
    def _check_files(self):
        ...
        if subprocess.call([VALGRIND, '-h'], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT) != 0:
            raise FailedCommand(
                'Unable to run valgrind.  Is it installed and in PATH?')
    ...
    def check_valgrind(self):
        ...
        if not self.mem_cleanup:
            self.logger.warning(
                'Memory still reachable at process end; shutdown not clean!')
    ...
    def start_tiny(self):
        ...
        cmd = [os.path.join(CURRENT_DIR, os.path.relpath(
            TINY, TINY_DIR)), str(self.tiny_port)]
        kwargs = {}
        if self.tiny_output:
    ...
    def start_proxy(self):
        ...
        if self.use_valgrind:
            cmd = ['valgrind', '--log-file=%s' %
                   (self.valgrind_log_file), '--leak-check=full', '-v'] + cmd
        kwargs = {}
    ...
    def start_nop_server(self):
        self.logger.info('Starting nop-server on port %d' %
                         self.nop_server_port)
    ...
    def download_noproxy(self, url, output, timeout):
        self.logger.info(
            'Fetching %s into %s - direct from tiny' % (url, output))
        # create an empty file
        open(output, 'w').close()
    ...
    def _is_newer_than(self, file1, file2):
        ...
        try:
            if os.path.getmtime(file1) > os.path.getmtime(file2):
                ...
                self.logger.error(
                    '%s should have been received before %s' % (file2, file1))
                return False
        except OSError as e:
...
class CacheTest(ProxyTest):
    ...
    def run(self):
        ...
        for (i, filename) in enumerate(self.FILES):
            ...
            url = 'http://%s:%d/%s' % (self.tiny_host,
                                       self.tiny_port, filename)
...
class ExtendedConcurrencyProxyTest(ProxyTest):
    ...
    def run(self):
        ...
        for i in range(i, times_to_run):
            ...
            noproxy_proc = self.download_noproxy(
                url, dst_path_noproxy, timeout)

            tried_slow.append(
                (proxy_proc, noproxy_proc, dst_path_proxy, dst_path_noproxy))

        time.sleep(3)

        self.num_processes_realtime = self.get_num_processes(
            self.proxy_proc.pid)
        ...
        for i in range(times_to_run, times_to_run * 2):
            ...
            tried.append((proxy_proc, noproxy_proc,
                         dst_path_proxy, dst_path_noproxy))
        ...
        for i in range(times_to_run):
            (proxy_proc_s, noproxy_proc_s, dst_path_proxy_s,
             dst_path_noproxy_s) = tried_slow[i]
            (proxy_proc, noproxy_proc, dst_path_proxy,
             dst_path_noproxy) = tried[i]
            same_s = self._are_same(dst_path_noproxy_s, dst_path_proxy_s)
            same = self._are_same(dst_path_noproxy, dst_path_proxy)
            fast_before_slow = self._is_newer_than(
                dst_path_proxy_s, dst_path_proxy)
